...
class Schema {
	...
	public function register_rest_fields() {
		...
		register_rest_field(
			...
			[
				'get_callback'    => function ( $data_object ) {
					...
					return $module->get_completion_message_body_format();
				},
				'update_callback' => function ( $value, $data_object ) {
					$module = Module::get_instance( $data_object->ID );
					if ( ! $module ) {
		...
		register_rest_field(
			...
			[
				'get_callback'    => function ( $data_object ) {
					...
					return $module->get_completion_message_subject();
				},
				'update_callback' => function ( $value, $data_object ) {
					$module = Module::get_instance( $data_object->ID );
					if ( ! $module ) {
		...
		register_rest_field(
			...
			[
				'get_callback'    => function ( $data_object ) {
					return Module::get_page_send_completion_email( $data_object['id'] );
				},
				'update_callback' => function ( $value, $data_object ) {
					$value = ! empty( $value ) ? '1' : '0';
